...
public class UpsService extends GenericService<Ups, UpsDTO> {
	 ...
	 */
	public UpsService(UpsRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 */
	private void validateForeignKeyReferences(UpsDTO dto) {
	 ...
	 */
	public List<UpsDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetUpssWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        upsDto.setPartid(parts.getPartid());
	        upsDto.setParts(parts);
	        upsDto.setCapacityW((Integer) r[3]);
	        upsDto.setCapacityVa((Integer) r[4]);
	...
	public boolean partialUpdate(Integer partid, UpsDTO dto) {
		...
		if (optionalEntity.isPresent()) {
			...
			if (dto.getCapacityW() != null) {
				entity.setCapacityW(dto.getCapacityW());
			}
			if (dto.getCapacityVa() != null) {
				entity.setCapacityVa(dto.getCapacityVa());
			}
	 ...
	 */ /*
	public List<UpsDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Ups> list = repository.findByPartID(partid);
