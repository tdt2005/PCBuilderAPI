...
public class OsService extends GenericService<Os, OsDTO> {
	 ...
	 */
	public OsService(OsRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 */
	private void validateForeignKeyReferences(OsDTO dto) {
	 ...
	 */
	public List<OsDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetOssWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        osDto.setPartid(parts.getPartid());
	        osDto.setParts(parts);
	        osDto.setMode((String) r[3]);
	        osDto.setMaxMemory((Integer) r[4]);
	 ...
	 */ /*
	public List<OsDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Os> list = repository.findByPartID(partid);
