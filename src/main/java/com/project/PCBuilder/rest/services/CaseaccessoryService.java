...
import com.project.PCBuilder.persistence.entities.Caseaccessory;
import com.project.PCBuilder.persistence.repositories.CaseaccessoryRepository;
...
/**
 * REST service for entity "Caseaccessory" <br>
 * 
...
@Service
@Transactional
public class CaseaccessoryService extends GenericService<Caseaccessory, CaseaccessoryDTO> {

	private static final Logger logger = LoggerFactory.getLogger(CaseaccessoryService.class);

	private final CaseaccessoryRepository repository; // injected by constructor
	 ...
	 */
	public CaseaccessoryService(CaseaccessoryRepository repository, PartsRepository partsRepository, EntityManager em) {
		super(Caseaccessory.class, CaseaccessoryDTO.class);
	 ...
	 * @return
	 */
	private Integer getEntityId(CaseaccessoryDTO dto) {
	 ...
	 */
	private void validateForeignKeyReferences(CaseaccessoryDTO dto) {
	 ...
	 */
	/*
	 * public List<CaseaccessoryDTO> findAll() {
			logger.debug("findAll()");
			Iterable<Caseaccessory> all = repository.findAll();
			return entityListToDtoList(all);
		}
	 */
	public List<CaseaccessoryDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetCaseaccessorysWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        CaseaccessoryDTO caseaccessoryDto = new CaseaccessoryDTO();
	        ...
	        caseaccessoryDto.setPartid(parts.getPartid());
	        caseaccessoryDto.setParts(parts);
	        caseaccessoryDto.setType((String) r[3]);
	        caseaccessoryDto.setFormFactor((String) r[4]);

	        return caseaccessoryDto;
	 ...
	 */
	public CaseaccessoryDTO findById(Integer partid) {
		Integer entityId = partid;
		logger.debug("findById({})", entityId);
		Optional<Caseaccessory> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(Integer partid, CaseaccessoryDTO dto) {
		Integer entityId = partid;
	 ...
	 */
	public boolean update(CaseaccessoryDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(Integer partid, CaseaccessoryDTO dto) {
		Integer entityId = partid;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		
		Optional<Caseaccessory> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Caseaccessory entity = optionalEntity.get();
	 ...
	 */
	public boolean create(CaseaccessoryDTO dto) {
	 ...
	 */
	/*
	public List<CaseaccessoryDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Caseaccessory> list = repository.findByPartID(partid);
...
/***
	...
	public List<CaseaccessoryDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		List<Caseaccessory> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<CaseaccessoryDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		List<Caseaccessory> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}
***/
}