...
import com.project.PCBuilder.persistence.repositories.PowersupplyRepository;
...
/**
 * REST service for entity "Powersupply" <br>
 * 
...
@Service
@Transactional
public class PowersupplyService extends GenericService<Powersupply, PowersupplyDTO> {

	private static final Logger logger = LoggerFactory.getLogger(PowersupplyService.class);

	private final PowersupplyRepository repository; // injected by constructor
	 ...
	 */
	public PowersupplyService(PowersupplyRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 * @return
	 */
	private Integer getEntityId(PowersupplyDTO dto) {
	 ...
	 */
	private void validateForeignKeyReferences(PowersupplyDTO dto) {
	 ...
	 * public List<PowersupplyDTO> findAll() {
			...
			Iterable<Powersupply> all = repository.findAll();
			return entityListToDtoList(all);
		}
	 */
	public List<PowersupplyDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetPowersupplysWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        powersupplyDto.setPartid(parts.getPartid());
	        powersupplyDto.setParts(parts);
	        powersupplyDto.setType((String) r[3]);
	        powersupplyDto.setEfficiency((String) r[4]);
	        powersupplyDto.setWattage((Integer) r[5]);
	        powersupplyDto.setModular((String) r[6]);
	        powersupplyDto.setColor((String) r[7]);
	 ...
	 */
	public PowersupplyDTO findById(Integer partid) {
		Integer entityId = partid;
		logger.debug("findById({})", entityId);
		Optional<Powersupply> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(Integer partid, PowersupplyDTO dto) {
		Integer entityId = partid;
	 ...
	 */
	public boolean update(PowersupplyDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(Integer partid, PowersupplyDTO dto) {
		Integer entityId = partid;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		
		Optional<Powersupply> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Powersupply entity = optionalEntity.get();
			...
			if (dto.getWattage() != null) {
				entity.setWattage(dto.getWattage());
			}
			if (dto.getModular() != null) {
				entity.setModular(dto.getModular());
			}
	 ...
	 */
	public boolean create(PowersupplyDTO dto) {
	 ...
	 */ /*
	public List<PowersupplyDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Powersupply> list = repository.findByPartID(partid);
...
/***
	...
	public List<PowersupplyDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		List<Powersupply> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<PowersupplyDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		List<Powersupply> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}
***/
}