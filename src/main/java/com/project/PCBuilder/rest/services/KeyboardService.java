...
public class KeyboardService extends GenericService<Keyboard, KeyboardDTO> {
	 ...
	 */
	public KeyboardService(KeyboardRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 */
	public List<KeyboardDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetKeyboardsWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        keyboardDto.setPartid(parts.getPartid());
	        keyboardDto.setParts(parts);
	        keyboardDto.setStyle((String) r[3]);
	        keyboardDto.setSwitches((String) r[4]);
	        keyboardDto.setBacklit((String) r[5]);
	        keyboardDto.setTenkeyless((Boolean) r[6]);
	        keyboardDto.setConnectionType((String) r[7]);
	        keyboardDto.setColor((String) r[8]);
