...
import com.project.PCBuilder.persistence.repositories.HeadphonesRepository;
...
/**
 * REST service for entity "Headphones" <br>
 * 
...
@Service
@Transactional
public class HeadphonesService extends GenericService<Headphones, HeadphonesDTO> {

	private static final Logger logger = LoggerFactory.getLogger(HeadphonesService.class);

	private final HeadphonesRepository repository; // injected by constructor
	 ...
	 */
	public HeadphonesService(HeadphonesRepository repository, PartsRepository partsRepository, EntityManager em) {
		super(Headphones.class, HeadphonesDTO.class);
	 ...
	 * @return
	 */
	private Integer getEntityId(HeadphonesDTO dto) {
	 ...
	 */
	private void validateForeignKeyReferences(HeadphonesDTO dto) {
	 ...
	 */
	/*
	 * public List<HeadphonesDTO> findAll() {
			logger.debug("findAll()");
			Iterable<Headphones> all = repository.findAll();
			return entityListToDtoList(all);
		}
	 */
	public List<HeadphonesDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetHeadphonessWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        headphonesDto.setPartid(parts.getPartid());
	        headphonesDto.setParts(parts);
	        headphonesDto.setType((String) r[3]);
	        headphonesDto.setFrequencyResponse((String) r[4]);
	        headphonesDto.setMicrophone((Boolean) r[5]);
	        headphonesDto.setWireless((Boolean) r[6]);
	        headphonesDto.setEnclosureType((String) r[7]);
	        headphonesDto.setColor((String) r[8]);
	 ...
	 */
	public HeadphonesDTO findById(Integer partid) {
		Integer entityId = partid;
		logger.debug("findById({})", entityId);
		Optional<Headphones> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(Integer partid, HeadphonesDTO dto) {
		Integer entityId = partid;
	 ...
	 */
	public boolean update(HeadphonesDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(Integer partid, HeadphonesDTO dto) {
		Integer entityId = partid;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		
		Optional<Headphones> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Headphones entity = optionalEntity.get();
			
			// Update only non-null fields from DTO
			if (dto.getType() != null) {
				entity.setType(dto.getType());
			}
			if (dto.getFrequencyResponse() != null) {
				entity.setFrequencyResponse(dto.getFrequencyResponse());
			}
			if (dto.getMicrophone() != null) {
				entity.setMicrophone(dto.getMicrophone());
			}
			if (dto.getWireless() != null) {
				entity.setWireless(dto.getWireless());
			}
			if (dto.getEnclosureType() != null) {
				entity.setEnclosureType(dto.getEnclosureType());
			}
	 ...
	 */
	public boolean create(HeadphonesDTO dto) {
	 ...
	 */ /*
	public List<HeadphonesDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Headphones> list = repository.findByPartID(partid);
...
/***
	...
	public List<HeadphonesDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		List<Headphones> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<HeadphonesDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		List<Headphones> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}
***/
}