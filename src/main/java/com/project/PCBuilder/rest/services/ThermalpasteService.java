...
public class ThermalpasteService extends GenericService<Thermalpaste, ThermalpasteDTO> {
	 ...
	 */
	public ThermalpasteService(ThermalpasteRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 */
	public List<ThermalpasteDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetThermalpastesWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ThermalpasteDTO thermalpasteDto = new ThermalpasteDTO();
	        ...
	        thermalpasteDto.setPartid(parts.getPartid());
	        thermalpasteDto.setParts(parts);
	        thermalpasteDto.setAmount((BigDecimal) r[3]);

	        return thermalpasteDto;
