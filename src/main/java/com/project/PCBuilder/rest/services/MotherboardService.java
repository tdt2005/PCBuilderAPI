...
public class MotherboardService extends GenericService<Motherboard, MotherboardDTO> {
	 ...
	 */
	public MotherboardService(MotherboardRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 */
	private void validateForeignKeyReferences(MotherboardDTO dto) {
	 ...
	 */
	public List<MotherboardDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetMotherboardsWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        motherboardDto.setPartid(parts.getPartid());
	        motherboardDto.setParts(parts);
	        motherboardDto.setSocket((String) r[3]);
	        motherboardDto.setFormFactor((String) r[4]);
	        motherboardDto.setMaxMemory((Integer) r[5]);
	        motherboardDto.setMemorySlots((Integer) r[6]);
	        motherboardDto.setColor((String) r[7]);
	...
	public boolean partialUpdate(Integer partid, MotherboardDTO dto) {
		...
		if (optionalEntity.isPresent()) {
			...
			if (dto.getMaxMemory() != null) {
				entity.setMaxMemory(dto.getMaxMemory());
			}
			if (dto.getMemorySlots() != null) {
				entity.setMemorySlots(dto.getMemorySlots());
			}
	 ...
	 */ /*
	public List<MotherboardDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Motherboard> list = repository.findByPartID(partid);
