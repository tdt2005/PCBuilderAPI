...
import com.project.PCBuilder.persistence.repositories.MouseRepository;
...
/**
 * REST service for entity "Mouse" <br>
 * 
...
@Service
@Transactional
public class MouseService extends GenericService<Mouse, MouseDTO> {

	private static final Logger logger = LoggerFactory.getLogger(MouseService.class);

	private final MouseRepository repository; // injected by constructor
	 ...
	 */
	public MouseService(MouseRepository repository, PartsRepository partsRepository, EntityManager em) {
		super(Mouse.class, MouseDTO.class);
	 ...
	 * @return
	 */
	private Integer getEntityId(MouseDTO dto) {
	 ...
	 */
	private void validateForeignKeyReferences(MouseDTO dto) {
	 ...
	 * public List<MouseDTO> findAll() {
			...
			Iterable<Mouse> all = repository.findAll();
			return entityListToDtoList(all);
		}
	 */
	public List<MouseDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetMousesWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        mouseDto.setPartid(parts.getPartid());
	        mouseDto.setParts(parts);
	        mouseDto.setTrackingMethod((String) r[3]);
	        mouseDto.setConnectionType((String) r[4]);
	        mouseDto.setMaxDpi((Integer) r[5]);
	        mouseDto.setHandOrientation((String) r[6]);
	        mouseDto.setColor((String) r[7]);
	 ...
	 */
	public MouseDTO findById(Integer partid) {
		Integer entityId = partid;
		logger.debug("findById({})", entityId);
		Optional<Mouse> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(Integer partid, MouseDTO dto) {
		Integer entityId = partid;
	 ...
	 */
	public boolean update(MouseDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(Integer partid, MouseDTO dto) {
		Integer entityId = partid;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		
		Optional<Mouse> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Mouse entity = optionalEntity.get();
			
			// Update only non-null fields from DTO
			if (dto.getTrackingMethod() != null) {
				entity.setTrackingMethod(dto.getTrackingMethod());
			}
			if (dto.getConnectionType() != null) {
				entity.setConnectionType(dto.getConnectionType());
			}
			if (dto.getMaxDpi() != null) {
				entity.setMaxDpi(dto.getMaxDpi());
			}
			if (dto.getHandOrientation() != null) {
				entity.setHandOrientation(dto.getHandOrientation());
			}
	 ...
	 */
	public boolean create(MouseDTO dto) {
	 ...
	 */ /*
	public List<MouseDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Mouse> list = repository.findByPartID(partid);
...
/***
	...
	public List<MouseDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		List<Mouse> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<MouseDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		List<Mouse> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}
***/
}