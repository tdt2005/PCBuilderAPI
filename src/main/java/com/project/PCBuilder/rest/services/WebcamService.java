...
import com.project.PCBuilder.persistence.entities.Webcam;
import com.project.PCBuilder.persistence.repositories.WebcamRepository;
...
/**
 * REST service for entity "Webcam" <br>
 * 
...
@Service
@Transactional
public class WebcamService extends GenericService<Webcam, WebcamDTO> {

	private static final Logger logger = LoggerFactory.getLogger(WebcamService.class);

	private final WebcamRepository repository; // injected by constructor
	 ...
	 */
	public WebcamService(WebcamRepository repository, PartsRepository partsRepository, EntityManager em) {
		super(Webcam.class, WebcamDTO.class);
	 ...
	 * @return
	 */
	private Integer getEntityId(WebcamDTO dto) {
	 ...
	 */
	private void validateForeignKeyReferences(WebcamDTO dto) {
	 ...
	 */
	/*
	 * public List<WebcamDTO> findAll() {
			logger.debug("findAll()");
			Iterable<Webcam> all = repository.findAll();
			return entityListToDtoList(all);
		}
	 */
	public List<WebcamDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetWebcamsWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        webcamDto.setPartid(parts.getPartid());
	        webcamDto.setParts(parts);
	        webcamDto.setResolutions((String) r[3]);
	        webcamDto.setConnection((String) r[4]);
	        webcamDto.setFocusType((String) r[5]);
	        webcamDto.setOs((String) r[6]);
	        webcamDto.setFov((BigDecimal) r[7]);
	 ...
	 */
	public WebcamDTO findById(Integer partid) {
		Integer entityId = partid;
		logger.debug("findById({})", entityId);
		Optional<Webcam> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(Integer partid, WebcamDTO dto) {
		Integer entityId = partid;
	 ...
	 */
	public boolean update(WebcamDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(Integer partid, WebcamDTO dto) {
		Integer entityId = partid;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		
		Optional<Webcam> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Webcam entity = optionalEntity.get();
			...
			if (dto.getFocusType() != null) {
				entity.setFocusType(dto.getFocusType());
			}
			if (dto.getOs() != null) {
				entity.setOs(dto.getOs());
			}
			if (dto.getFov() != null) {
				entity.setFov(dto.getFov());
			}
	 ...
	 */
	public boolean create(WebcamDTO dto) {
	 ...
	 */ /*
	public List<WebcamDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Webcam> list = repository.findByPartID(partid);
...
/***
	...
	public List<WebcamDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		List<Webcam> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<WebcamDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		List<Webcam> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}
***/
}