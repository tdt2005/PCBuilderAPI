...
import com.project.PCBuilder.persistence.entities.Memory;
import com.project.PCBuilder.persistence.repositories.MemoryRepository;
...
/**
 * REST service for entity "Memory" <br>
 * 
...
@Service
@Transactional
public class MemoryService extends GenericService<Memory, MemoryDTO> {

	private static final Logger logger = LoggerFactory.getLogger(MemoryService.class);

	private final MemoryRepository repository; // injected by constructor
	 ...
	 */
	public MemoryService(MemoryRepository repository, PartsRepository partsRepository, EntityManager em) {
	 ...
	 * @return
	 */
	private Integer getEntityId(MemoryDTO dto) {
	 ...
	 */
	private void validateForeignKeyReferences(MemoryDTO dto) {
	 ...
	 * public List<MemoryDTO> findAll() {
			...
			Iterable<Memory> all = repository.findAll();
			return entityListToDtoList(all);
		}
	 */
	public List<MemoryDTO> findAll(int pageNumber, int pageSize, String searchName, BigDecimal minPrice, BigDecimal maxPrice, String sortBy, String sortDirection) {
	    logger.debug("findAll() via stored procedure with parameters");

	    StoredProcedureQuery sp = em.createStoredProcedureQuery("sp_GetMemorysWithPagination");
	    ...
	    return rows.stream().map(r -> {
	        ...
	        memoryDto.setPartid(parts.getPartid());
	        memoryDto.setParts(parts);
	        memoryDto.setSpeed((String) r[3]);
	        memoryDto.setModules((String) r[4]);
	        memoryDto.setPricePerGb((BigDecimal) r[5]);
	        memoryDto.setColor((String) r[6]);
	        memoryDto.setFirstWordLatency((BigDecimal) r[7]);
	        memoryDto.setCasLatency((BigDecimal) r[8]);
	 ...
	 */
	public MemoryDTO findById(Integer partid) {
		Integer entityId = partid;
		logger.debug("findById({})", entityId);
		Optional<Memory> optionalEntity = repository.findById(entityId);
	 ...
	 * @param dto 
	 */
	public void save(Integer partid, MemoryDTO dto) {
		Integer entityId = partid;
	 ...
	 */
	public boolean update(MemoryDTO dto) {
	 ...
	 * @return true if updated, false if not found
	 */
	public boolean partialUpdate(Integer partid, MemoryDTO dto) {
		Integer entityId = partid;
		logger.debug("partialUpdate({}, {})", entityId, dto);
		
		Optional<Memory> optionalEntity = repository.findById(entityId);
		if (optionalEntity.isPresent()) {
			Memory entity = optionalEntity.get();
			...
			if (dto.getFirstWordLatency() != null) {
				entity.setFirstWordLatency(dto.getFirstWordLatency());
			}
			if (dto.getCasLatency() != null) {
				entity.setCasLatency(dto.getCasLatency());
			}
	 ...
	 */
	public boolean create(MemoryDTO dto) {
	 ...
	 */ /*
	public List<MemoryDTO> findByParts(Integer partid) {
		logger.debug("findByParts({})", partid);
		List<Memory> list = repository.findByPartID(partid);
...
/***
	...
	public List<MemoryDTO> findByTitle(String title) {
		logger.debug("findByTitle({})", title);
		List<Memory> list = repository.findByTitleContaining(title);
		return entityListToDtoList(list);
	}

	public List<MemoryDTO> findByPrice(BigDecimal price) {
		logger.debug("findByPrice({})", price);
		List<Memory> list = repository.findByPrice(price);
		return entityListToDtoList(list);
	}
***/
}