...
/**
 * Spring Data JPA repository for entity "Pcs" <br>
 *
...
public interface PcsRepository extends JpaRepository<Pcs, Integer> {
    ...
    // List<Pcs> findByXxx(String xxx);

    // Example: Find by a field with LIKE behavior
    // List<Pcs> findByXxxContaining(String xxx);

    // Example: Find by a numerical field
    // List<Pcs> findByYyy(BigDecimal yyy);

    // Example: Find by foreign key (like PartID)
    // List<Pcs> findByPartID(Integer partID); // Use Optional<> or single result for OneToOne
